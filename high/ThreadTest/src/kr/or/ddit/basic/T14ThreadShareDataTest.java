...
public class T14ThreadShareDataTest {
	...
	// 공유객체,,, 여러개의 스레드가 동작할때 스레드끼리 서로 정보를 주고받아야 되는 상황에 사용
...
class CalcPIThread extends Thread {
	...
	public void run() {
		 ...
		 */
		double sum = 0.0;
		for(int i=1; i<=1500000000; i+=2) { //i값을 분모로 사용할거라 2씩 더함
			if( ((i/2) % 2) == 0) {		// 2로 나눈 몪이 짝수이면 부호가 +
				sum += (1.0 / i);
...
class PrintThread extends Thread {
	...
	public void run() {
		while(true) {
			...
			if(sd.isOk()) { // setOK가 true가 되면 무한루프 빠져나와서 계산된 원주율 출력
				break;
			}
		}
