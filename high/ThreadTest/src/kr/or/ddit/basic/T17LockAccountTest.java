...
public class T17LockAccountTest {
	...
	public static void main(String[] args) {
		...
		ReentrantLock lock = new ReentrantLock(true); // 락 객체 생성. fairness를 생성자로 설정
		
		LockAccount lAcc = new LockAccount(lock);
		lAcc.deposit(10000); //만원 입금
...
class LockAccount {
	...
	// Lock 객체 변수 생성 => 되도록이면 private final로 설정한다.
	...
	public void deposit(int money) {
		// Lock객체의 lock()메소드가 동기화의 시작이고, unlock()메서드가 동기화의 끝을 나타낸다.
		// lock() 메서드로 동기화를 설정한 곳에서는 반드시 unlock()메서드로 해제해야 한다.
