...
//	public enum HomeTown {서울, 부산, 대구, 광주, 대전, 전북, 경기도};
	...
	public static void main(String[] args) {
		...
		Season ss = Season.valueOf("여름"); // = Season.여름;
		...
		for(City city : City.values()) { // values => '배열'이기 때문에 향상된 for문 사용 가능
...
//		System.out.println(City.대전 == HomeTown.대전); // enum의 장점! 컴파일러가 잘못된 비교임을 알려줌(type safe한 코드 작성 가능)
													 //				같은 타입이 아니면 비교 자체를 못하게 막아버림
		
		System.out.println("-----------------------------------------");
		
		System.out.println("대구 => " + city.compareTo(City.대구)); // comparable을 implement한 객체
		System.out.println("서울 => " + city.compareTo(City.서울)); // ordinal값을 가지고 뭐가 더 큰지 비교... 정렬 기능
