...
public class T05ServletCookieTest extends HttpServlet {
	...
	/*
	 	...
	 	- 경로: 쿠키를 공유할 기준 경로를 지정한다. (지정하지 않으면 실행한 URL의 경로 부분이 사용됨)
	 	
	 	2. 동작 방식
	 	- 쿠키 생성 단계: 생성한 쿠키를 응답데이터의 헤더에 저장하여 웹브라우저에 전송한다.
	 	- 쿠키 저장 단계: 웹브라우저는 응답데이터에 포함된 쿠키를 쿠키저장소에 보관한다.
	 	- 쿠키 전송 단계: 웹브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에 전송한다.(삭제 전까지)
...
//		readCookieExam(req, resp); // 쿠키 정보 읽기 예제
	...
	/*
	 	...
	 	2. 쿠키객체의 최대 지속 시간을 0(초)으로 설정한다.★
	 	3. 설정한 쿠키객체를 응답헤더에 추가하여 전송한다. (addCookie)
	...
	/*
	 	...
	 	1. 쿠키 객체 생성하기. 사용 불가 문자(공백, []()=,"/?@:;)
	 	   Cookie cookie = new Cookie("key값", "value값");
	 	       쿠키값은 사용불가 문자를 제외한 나머지 출력 가능한 아스키 문자 사용 가능
	 	  	 => 이외의 값(ex:한글)을 사용하는 경우 URLEncoder.encode()를 사용하여 처리
	 	
	 	2. 쿠키 최대 지속시간 설정하기(초단위) => 지정하지 않으면 브라우저 종료 시 삭제
